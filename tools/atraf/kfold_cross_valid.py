...
#BE
def by_session_wrap_dataset(input_dataset, fold, num_splits, test_mode = False, extra_params = None):
    #TODO: implement
...
def parse_args():
    ...
    parser.add_argument('--wrap_dataset_func', type=str, default ='wrap_dataset',
                        help='User function to wrap dataset in k-fold')
...
def train_single_fold(cfg, num_splits, fold, resume_ckpt=None, wrap_dataset_func=None):
    ...
    def call_correct_wrap_dataset(in_dataset, test_mode, wrap_dataset_func):

        # TODO: call user function for wrap_dataset
        if (wrap_dataset_func == 'wrap_dataset'):
            temp_dataset = wrap_dataset(in_dataset, False)
        elif (wrap_dataset_func == 'whole_dataset'):
            temp_dataset = in_dataset
        elif (wrap_dataset_func == 'by_session_wrap_dataset'):
            temp_dataset = by_session_wrap_dataset(in_dataset, test_mode=False,
                                                   fold = fold, num_splits = num_splits,
                                                   seed=cfg.kfold_split_seed,
                                                   extra_params=None)
        else:
            raise ValueError('wrap_dataset_func is unknown')

        return temp_dataset

    train_dataset = copy.deepcopy(dataset)

    cfg.train_dataloader.dataset = call_correct_wrap_dataset(train_dataset, test_mode=False, wrap_dataset_func=wrap_dataset_func)
    if cfg.val_dataloader is not None:
        ...
        cfg.val_dataloader.dataset =  call_correct_wrap_dataset(val_dataset, test_mode=True, wrap_dataset_func=wrap_dataset_func)
    if cfg.test_dataloader is not None:
        ...
        cfg.test_dataloader.dataset = call_correct_wrap_dataset(val_dataset, test_mode=True, wrap_dataset_func=wrap_dataset_func)
...
def main():
    ...
    wrap_dataset_func = args.wrap_dataset_func

    for fold in folds:
        cfg_ = copy.deepcopy(cfg)
        train_single_fold(cfg_, args.num_splits, fold, resume_ckpt,wrap_dataset_func)
        resume_ckpt = None
